...
public class WaitingComparator implements Comparator<StudentEntry> {
	...
	public int compare(StudentEntry arg0, StudentEntry arg1) {
		...
		/*	
		 * Score:
		 * 1 - Studenten har egentlig ikke lov til � slippe inn
		 * 2 - Studenten har lov til � slippe inn
		 * 3 - Studenten har lov til � slippe inn og har satt seg p� venteliste p� nett
		 * 4 - Studenten er VIP
		 */
		...
		else { // m�tt opp og meldt seg p� ventelista
			score0 = 2;
		}
		...
		//Sjekker p� dato eller venteliste alt ettersom hvis de er like.
		if(score1==score0){
			//De er like		
			if(score1 == 3){
				//Sorter p� plass i ventelista
				return logic.getEvent().getNetWaitingList().indexOf(stud0)-logic.getEvent().getNetWaitingList().indexOf(stud1);
			} else {
				//Sorter p� dato de var lagt til
